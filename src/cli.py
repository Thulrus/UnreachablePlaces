...
def preprocess(ctx):
    ...
    try:
        ...
        if not boundary_file.exists() or not roads_file.exists():
            click.echo("✗ Data not found. Please run 'fetch_data' first.", err=True)
            return 1
...
def compute_distance(ctx):
    ...
    try:
        ...
        if not boundary_file.exists() or not road_mask_file.exists():
            click.echo("✗ Preprocessed data not found. Please run 'preprocess' first.", err=True)
            return 1
...
def find_unreachable(ctx):
    ...
    try:
        ...
        if not distance_file.exists():
            click.echo("✗ Distance field not found. Please run 'compute_distance' first.", err=True)
            return 1
        ...
        click.echo(f"  Location: {results['most_unreachable_point']['latitude']:.6f}, "
                  f"{results['most_unreachable_point']['longitude']:.6f}")
        click.echo(f"  Distance: {results['most_unreachable_point']['distance_km']:.2f} km")
...
def visualize(ctx):
    ...
    try:
        ...
        results_file = config.get('output.results_file', 'outputs/results.json')
        
        if not all([Path(f).exists() for f in [distance_file, boundary_file, results_file]]):
            click.echo("✗ Required data not found. Please run previous steps first.", err=True)
            return 1
        ...
        boundary = gpd.read_file(boundary_file)
        roads = gpd.read_file(roads_file) if roads_file.exists() else gpd.GeoDataFrame()
        ...
        outputs = visualizer.visualize_all(distance_data, processed_data, results)
...
@cli.command()
@click.option('--skip-fetch', is_flag=True, help='Skip data fetching if already downloaded')
@click.pass_context
def run_all(ctx, skip_fetch):
    ...
    try:
        ...
        if not skip_fetch:
            ...
            data = {
                'boundary': gpd.read_file(raw_data_path / f"{state_name}_boundary.geojson"),
                'roads': gpd.read_file(raw_data_path / f"{state_name}_roads.geojson")
            }
        ...
        click.echo(f"    Location: {results['most_unreachable_point']['latitude']:.6f}, "
                  f"{results['most_unreachable_point']['longitude']:.6f}")
        click.echo(f"    Distance: {results['most_unreachable_point']['distance_km']:.2f} km from nearest road")
