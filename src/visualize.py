...
class Visualizer:
    ...
    def create_static_map(self, distance_field: np.ndarray,
        ...
        if output_path is None:
            ...
            output_path = self.config.get_path('maps') / f"{state_name}_unreachability_map.png"
        ...
        cbar.set_label('Distance from Road (km)', rotation=270, labelpad=20, fontsize=12)
        
        # Plot boundary
        boundary.boundary.plot(ax=ax, color='black', linewidth=2, label='State Boundary')
        
        # Plot roads (sample if too many)
        if len(roads) > 10000:
            roads_sample = roads.sample(n=10000, random_state=42)
            print(f"  Plotting sample of {len(roads_sample)} roads (out of {len(roads)})")
        else:
            roads_sample = roads
        
        roads_sample.plot(ax=ax, color='black', linewidth=0.3, alpha=0.3, label='Roads')
        
        # Plot unreachable point
        point_x = unreachable_point['x_projected']
        point_y = unreachable_point['y_projected']
        
        ax.plot(point_x, point_y, 'r*', markersize=20, 
                markeredgecolor='white', markeredgewidth=1.5,
                label='Most Unreachable Point', zorder=5)
        
        # Add annotation
        distance_km_val = unreachable_point['distance_km']
        ax.annotate(
            f"{distance_km_val:.2f} km from nearest road",
            xy=(point_x, point_y),
            xytext=(15, 15),
    ...
    def create_interactive_map(self, distance_field: np.ndarray,
                              boundary: gpd.GeoDataFrame,
                              roads: gpd.GeoDataFrame,
                              unreachable_point: Dict,
                              results: Dict,
        ...
        if output_path is None:
            state_name = self.config.state_name.lower()
            output_path = self.config.get_path('maps') / f"{state_name}_unreachability_interactive.html"
        ...
        unreachable_layer = folium.FeatureGroup(name='Most Unreachable Point', show=True)
        ...
        folium.Marker(
            ...
            tooltip=f"Most Unreachable: {unreachable_point['distance_km']:.2f} km from road",
            icon=folium.Icon(color='red', icon='star', prefix='fa')
        ).add_to(unreachable_layer)
        
        unreachable_layer.add_to(m)
        
        # Add top 10 unreachable points
        top10_layer = folium.FeatureGroup(name='Top 10 Unreachable Points', show=False)
    ...
    def visualize_all(self, distance_data: dict, processed_data: dict, 
        ...
        if self.config.get('output.static_map', True):
            ...
            static_path = self.create_static_map(
                distance_field,
                boundary,
                roads,
                unreachable_point,
