...
def extract_from_national_file(national_file: Path, 
    ...
    try:
        ...
        print(f"  ✗ gdalwarp not found. Install GDAL: sudo apt install gdal-bin")
        return False
...
def ensure_terrain_data(config, state_name: str) -> tuple[Path | None, Path | None]:
    """
        ...
        if not dem_path.exists():
            ...
            if local_dem:
                ...
                if local_dem_path.exists():
                    print(f"DEM not found for {state_name}, extracting from {local_dem_path.name}...")
                    if extract_from_national_file(local_dem_path, boundary_path, dem_path):
                        dem_final = dem_path
                    else:
                        print("  Warning: Extraction failed, continuing without DEM (flat terrain assumed)")
                else:
                    print(f"  Warning: Local DEM file not found: {local_dem}")
                    print("  Continuing without DEM (flat terrain assumed)")
            else:
                print(f"  Warning: DEM not found for {state_name} and no local source configured")
                print("  Continuing without DEM (flat terrain assumed)")
        else:
            dem_final = dem_path
    else:
    ...
    landcover_enabled = config.get('cost_distance.terrain_data.landcover.enabled', True)
    landcover_final = None
    
    if landcover_enabled:
        if not landcover_path.exists():
            # Try local extraction first
            local_landcover = config.get('cost_distance.terrain_data.landcover.local_file')
            if local_landcover:
                local_lc_path = Path(local_landcover)
                if local_lc_path.exists():
                    print(f"Land cover not found for {state_name}, extracting from {local_lc_path.name}...")
                    if extract_from_national_file(local_lc_path, boundary_path, landcover_path):
                        landcover_final = landcover_path
                    else:
                        print("  Warning: Extraction failed, continuing without land cover (uniform cost)")
                else:
                    print(f"  Warning: Local land cover file not found: {local_landcover}")
                    print("  Continuing without land cover (uniform cost)")
            else:
                print(f"  Warning: Land cover not found for {state_name} and no local source configured")
                print("  Continuing without land cover (uniform cost)")
        else:
            landcover_final = landcover_path
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Extract terrain data for a state')
    parser.add_argument('--config', default='config.yaml', help='Path to config file')
    parser.add_argument('state', nargs='?', help='State name (optional, uses config if not provided)')
