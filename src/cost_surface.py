...
def slope_cost_factor(slope_degrees: np.ndarray, config: dict) -> np.ndarray:
    ...
    cost[mask] = flat_cost + (moderate_cost - flat_cost) * (slope_degrees[mask] / 15.0)
    
    # 15° to 30°
    mask = (slope_degrees > 15) & (slope_degrees <= 30)
    cost[mask] = moderate_cost + (steep_cost - moderate_cost) * ((slope_degrees[mask] - 15) / 15.0)
    
    # 30° to 45°
    mask = (slope_degrees > 30) & (slope_degrees <= 45)
    cost[mask] = steep_cost + (very_steep_cost - steep_cost) * ((slope_degrees[mask] - 30) / 15.0)
...
class CostSurfaceGenerator:
    ...
    def calculate_slope(self, dem_path: str, output_path: Optional[str] = None) -> np.ndarray:
        """
        ...
        logger.info(f"Slope range: {slope_degrees.min():.1f}° to {slope_degrees.max():.1f}°")
        return slope_degrees
    
    def resample_landcover(self, landcover_path: str, template_path: str, 
    ...
    def generate_cost_surface(self, dem_path: str, landcover_path: str,
        ...
        cost_surface = (slope_costs ** self.slope_weight) * (landcover_costs ** self.landcover_weight)
        
        # Ensure minimum cost of 1.0
        cost_surface = np.maximum(cost_surface, 1.0)
        
        logger.info(f"Cost surface range: {cost_surface.min():.2f} to {cost_surface.max():.2f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate cost surfaces for distance analysis")
