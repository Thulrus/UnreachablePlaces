...
class DataPreprocessor:
    ...
    def reproject_data(self, gdf: gpd.GeoDataFrame, target_crs: Optional[str] = None) -> gpd.GeoDataFrame:
        """
    ...
    def clip_to_boundary(self, roads: gpd.GeoDataFrame, boundary: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        print(f"Clipped {len(roads)} roads to {len(clipped)} road segments within boundary")
        
        return clipped
    
    def create_raster_grid(self, boundary: gpd.GeoDataFrame, 
        ...
        print(f"Created raster grid: {width}x{height} pixels at {resolution}m resolution")
    ...
    def save_raster(self, array: np.ndarray, metadata: dict, output_path: Path):
        """
    ...
    def preprocess_all(self, data: dict) -> dict:
        ...
        if self.config.get('output.save_intermediate', True):
            ...
            boundary.to_file(processed_dir / f"{state_name}_boundary_projected.geojson")
            roads_clipped.to_file(processed_dir / f"{state_name}_roads_clipped.geojson")
