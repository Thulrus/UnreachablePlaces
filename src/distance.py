...
class DistanceCalculator:
    ...
    def compute_distance_field(self,
        ...
        print(
            f"  Max distance: {max_distance:.2f} m ({max_distance/1000:.2f} km)"
        )
        print(
            f"  Mean distance: {mean_distance:.2f} m ({mean_distance/1000:.2f} km)"
        )

        return distance_meters

    def compute_cost_distance_field(
            self,
            mask: np.ndarray,
        ...
        print(
            f"  Max cost-distance: {max_distance:.2f} m ({max_distance/1000:.2f} km)"
        )
        print(
            f"  Mean cost-distance: {mean_distance:.2f} m ({mean_distance/1000:.2f} km)"
        )
        print(
            f"  Note: Cost-distance is 'effective distance' accounting for terrain"
        )
    ...
    def create_boundary_mask(self, boundary_gdf, raster_shape: Tuple[int, int],
        ...
        boundary_mask = rasterize(shapes=shapes,
    ...
    def save_distance_raster(self, distance_field: np.ndarray, metadata: dict,
                             output_path: Path):
        """
    ...
    def compute_all(self, processed_data: dict) -> dict:
        ...
        if use_cost_distance:
            ...
            if not cost_surface_path.exists():
                print(
                    f"Warning: Cost surface not found at {cost_surface_path}")
                print("Falling back to Euclidean distance...")
                print(
                    "Run 'cost-surface' command first to enable cost-distance mode."
                )
        ...
        if use_cost_distance:
            ...
            distance_field = self.compute_cost_distance_field(
                road_mask, cost_surface)
        else:
        ...
        boundary_mask = self.create_boundary_mask(
            boundary, (metadata['height'], metadata['width']),
            metadata['transform'])
